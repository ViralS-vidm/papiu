...
@section('title') {{ __('booking::labels.service_request') }} @endsection
...
@section('content')
    ...
    @component('common-components.breadcrumb')
        ...
        @slot('li_2') {{ __('booking::labels.service_request') }} @endslot
    @endcomponent
                            ...
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <input type="text"
                                           ...
                                           value="{{ $search  }}"
                                           @endif
                                           onkeypress="return search(event, '{{ route('service-requests.index') }}')"
                                    />
                                    <i class="bx bx-search-alt search-icon"
                                       onclick="return redirectWithSearch('{{ route('service-requests.index') }}')"></i>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-8">
                            <div class="text-sm-right">
                                <a type="button" style="color: white;" href="{{ route('service-requests.create') }}"
                                   class="btn btn-success btn-rounded waves-effect waves-light mb-2 mr-2"><i
                                        class="mdi mdi-plus mr-1"></i> {{ __('core::labels.add_new') . ' ' . __('booking::labels.service_request') }}
                                </a>
                            </div>
                        </div><!-- end col-->
                    </div>
                        ...
                        <table class="table table-centered table-nowrap">
                            ...
                            @foreach($serviceRequests as $serviceRequest)
                                <tr>
                                    ...
                                    <td>
                                        <label class="badge badge-success">{{ $serviceRequest->contact_name }}</label>
                                        <label class="badge badge-success">{{ $serviceRequest->contact_email }}</label>
                                        <label class="badge badge-success">{{ $serviceRequest->contact_number }}</label>
                                    </td>
                                    <td>{{ $serviceRequest->statusName }}</td>
                                    <td>
                                        @if(! $serviceRequest->is_completed)

                                            {!! Form::open([
                                                    'method' => 'PATCH',
                                                    'route' => ['service-requests.complete', $serviceRequest->id],
                                            ...
                                            <a href="{{ route('service-requests.edit', [$serviceRequest->id]) }}"
                                            ...
                                            {!! Form::open([
                                                    'method' => 'DELETE',
                                                    'route' => ['service-requests.destroy', $serviceRequest->id],
                                                    'style'=>'display:inline',
