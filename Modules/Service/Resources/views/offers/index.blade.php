...
@section('title') {{ __('service::labels.offer') }} @endsection
...
@section('content')
    ...
    @component('common-components.breadcrumb')
        ...
        @slot('li_2') {{ __('service::labels.offer') }} @endslot
    @endcomponent
                            ...
                            <div class="search-box mr-2 mb-2 d-inline-block">
                                <div class="position-relative">
                                    <input type="text"
                                           ...
                                           value="{{ $search  }}"
                                           @endif
                                           onkeypress="return search(event, '{{ route('offers.index') }}')"
                                    />
                                    <i class="bx bx-search-alt search-icon"
                                       onclick="return redirectWithSearch('{{ route('offers.index') }}')"></i>
                                </div>
                            </div>
                        </div>
                        <div class="col-sm-8">
                            <div class="text-sm-right">
                                <a type="button" style="color: white;" href="{{ route('offers.create') }}"
                                   class="btn btn-success btn-rounded waves-effect waves-light mb-2 mr-2"><i
                                            class="mdi mdi-plus mr-1"></i> {{ __('core::labels.add_new') . ' ' . __('service::labels.offer') }}
                                </a>
                            </div>
                        </div><!-- end col-->
                    </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-centered table-nowrap">
                            ...
                            <tr>
                                ...
                                <th>{{ __('service::labels.content') }}</th>
                            ...
                            @foreach($offers as $offer)
                                <tr>
                                    <td>{{ $offer->name }}</td>
                                    <td>{!! wrap_long_text($offer->description) !!}</td>
                                    <td>{!! wrap_long_text($offer->content) !!}</td>
                                    <td>
